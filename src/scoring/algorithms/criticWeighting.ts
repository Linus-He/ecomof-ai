...
import { DEFAULT_CRITIC_OPTIONS, normalizeWeightMap } from "../types/scoringTypes"
...
function descriptorValues(matrix, descriptor) {
  return (matrix || [])
    ...
    .map(row => Number(row.values?.[descriptor.key]))
...
export function computeCriticWeighting({
  ...
  if (!matrix.length || keys.length === 0) {
    return {
      ...
      diagnostics: { method: "critic", fallbackUsed: true },
  ...
  const conflictScore = Object.fromEntries(descriptors.map(descriptor => [
    descriptor.key,
    descriptors.reduce((sum, other) => sum + (conflictMatrix[descriptor.key]?.[other.key] || 0), 0),
  ]))
  const informationContent = Object.fromEntries(descriptors.map(descriptor => [
    descriptor.key,
    (sigma[descriptor.key] || 0) * (conflictScore[descriptor.key] || 0),
  ]))
  ...
  if (!Number.isFinite(informationTotal) || informationTotal <= 0 || matrix.length < 2) {
    fallbackUsed = true
    weights = normalizeWeightMap({}, descriptors)
