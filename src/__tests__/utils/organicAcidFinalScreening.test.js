...
describe("organic acid final screening", () => {
  ...
  it("records the Organic Acid Knowledge Base and planned roadmap", () => {
    ...
    expect(versionDocs.currentVersion).toBe("V3.9")
    expect(versionDocs.completedRange).toBe("V1.0-V3.9")
    expect(versionDocs.versions.map(row => row.version)).toEqual(["V1.0", "V1.1", "V1.2", "V1.3", "V1.4", "V1.5", "V1.6", "V1.7", "V2.0-A", "V2.0-B", "V2.0-C", "V2.0-D", "V2.0-E", "V2.0-F", "V2.0-G", "V2.0-H", "V2.0-I", "V2.0-J", "V2.0-K", "V2.0-L", "V2.6", "V2.7", "V2.8", "V3.0", "V3.1", "V3.2", "V3.3", "V3.4", "V3.5", "V3.6", "V3.8", "V3.9"])
  ...
  it("records the V1.6 small real dataset, V1.7 trace workbench, and Knowledge Base upgrade", () => {
    ...
    expect(versionDocs.versions.find(row => row.version === "V3.9").status).toBe("current")
    expect(versionDocs.roadmap.map(row => [row.version, row.status])).toEqual([
      ["V2.1", "planned"],
    ])
  })
