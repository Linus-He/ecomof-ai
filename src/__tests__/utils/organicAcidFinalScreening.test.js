...
describe("organic acid final screening", () => {
  ...
  it("records the Organic Acid Knowledge Base and planned roadmap", () => {
    ...
    expect(versionDocs.currentVersion).toBe("V3.9.10")
    expect(versionDocs.completedRange).toBe("V1.0-V3.9.10")
    expect(versionDocs.versions.map(row => row.version)).toEqual(["V1.0", "V1.1", "V1.2", "V1.3", "V1.4", "V1.5", "V1.6", "V1.7", "V2.0-A", "V2.0-B", "V2.0-C", "V2.0-D", "V2.0-E", "V2.0-F", "V2.0-G", "V2.0-H", "V2.0-I", "V2.0-J", "V2.0-K", "V2.0-L", "V2.6", "V2.7", "V2.8", "V3.0", "V3.1", "V3.2", "V3.3", "V3.4", "V3.5", "V3.6", "V3.8", "V3.9", "V3.9.1", "V3.9.2", "V3.9.3", "V3.9.4", "V3.9.5", "V3.9.5.1", "V3.9.5.2", "V3.9.5.3", "V3.9.5.4", "V3.9.5.5", "V3.9.6", "V3.9.7", "V3.9.8", "V3.9.9", "V3.9.10"])
    ...
    expect(versionDocs.versions.find(row => row.version === "V3.9.5")).toEqual(expect.objectContaining({
      title: "Organic Acid Algorithm Flow Network UX Refactor",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.5.1")).toEqual(expect.objectContaining({
      title: "Organic Acid Stepwise Algorithm Execution Chain UX Fix",
      status: "completed",
      evidenceBoundary: expect.stringMatching(/not final catalytic proof/),
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.6")).toEqual(expect.objectContaining({
      title: "Organic Acid Real Data Binding and Preregistered Scoring",
      status: "completed",
      evidenceBoundary: expect.stringMatching(/does not hard-code a final winner/),
    }))
    ...
    expect(versionDocs.versions.find(row => row.version === "V3.3")).toEqual(expect.objectContaining({
      title: "Real Data Ingestion Program",
      relatedSection: "#methodology-algorithm-validation",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.6")).toEqual(expect.objectContaining({
      title: "Experimental Label Expansion and Robustness Validation",
      relatedSection: "#methodology-algorithm-validation",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.8")).toEqual(expect.objectContaining({
      title: "Research Validation Loop Closure",
      relatedSection: "#organic-acid-research-validation",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9")).toEqual(expect.objectContaining({
      title: "Global Database-Driven Dashboard Refactor",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.1")).toEqual(expect.objectContaining({
      title: "Full Per-Tab Database Card Wiring",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.2")).toEqual(expect.objectContaining({
      title: "Organic Acid Host-Guest Algorithm Foundation and UI Refactor",
      status: "completed",
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.3")).toEqual(expect.objectContaining({
      title: "Organic Acid Evidence Completion, Sensitivity/Ablation Analysis, and Formula Alignment",
      status: "completed",
      evidenceBoundary: expect.stringMatching(/Seed\/proxy\/curated\/inferred\/mixed/),
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.4")).toEqual(expect.objectContaining({
      title: "Organic Acid Experimental Activation Package and Algorithm Methodology",
      status: "completed",
      evidenceBoundary: expect.stringMatching(/Planning-ready seed\/proxy\/curated/),
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.5")).toEqual(expect.objectContaining({
      title: "Organic Acid Algorithm Flow Network UX Refactor",
      status: "completed",
      evidenceBoundary: expect.stringMatching(/UX and builder refactor only/),
    }))
    expect(versionDocs.versions.find(row => row.version === "V3.9.5.1")).toEqual(expect.objectContaining({
      title: "Organic Acid Stepwise Algorithm Execution Chain UX Fix",
      status: "completed",
  ...
  it("records the V1.6 small real dataset, V1.7 trace workbench, and Knowledge Base upgrade", () => {
    ...
    const v20h = versionDocs.versions.find(row => row.version === "V2.0-H")
    expect(v20h.status).toBe("completed")
    expect(v20h.title).toMatch(/Small-Sample Validation and Sensitivity Audit/)
    expect(v20h.keyUpdates.join(" ")).toMatch(/verification queue/i)
    ...
    expect(versionDocs.versions.find(row => row.version === "V3.2").status).toBe("completed")
    expect(versionDocs.versions.find(row => row.version === "V3.3").status).toBe("completed")
    expect(versionDocs.versions.find(row => row.version === "V3.4").status).toBe("completed")
    expect(versionDocs.versions.find(row => row.version === "V3.5").status).toBe("completed")
    ...
    expect(versionDocs.versions.find(row => row.version === "V3.9.9").status).toBe("completed")
    expect(versionDocs.versions.find(row => row.version === "V3.9.10").status).toBe("current")
    expect(versionDocs.roadmap.map(row => [row.version, row.status])).toEqual([
      ["V2.1", "planned"],
    ])
  })
