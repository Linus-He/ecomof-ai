...
describe("organic acid experimental activation package", () => {
  it("builds planning-ready host, Mo strategy, matrix, template, feedback, and readiness outputs", () => {
    const workbench = buildExperimentalActivationWorkbench(input, { topRoute: hostGuestRoutes[0] })

    expect(workbench.version).toBe("V3.9.4")
    expect(workbench.routeContext.routeId).toBe("route-al-mof-mo")
    expect(workbench.routeContext.topRouteName).toMatch(/Al-MOF \+ Mo/)
