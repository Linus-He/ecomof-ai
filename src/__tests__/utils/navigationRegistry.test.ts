...
describe("navigation registry", () => {
  ...
  it("resolves legacy aliases and nested scroll targets without duplicating maps", () => {
    ...
    expect(resolveTabForHash("project-evolution-version-timeline")).toBe("projectEvolution")
  ...
  it("registers the first-priority centers and the independent charter as real routes", () => {
    expect(["catalysisLiterature", "organicAcid", "algorithmValidation", "dataQuality", "benchmarkReferences", "mofRecord", "literatureRecord", "charter", "creatorStatement", "releaseNotes", "contact", "acknowledgements"].every(id => Boolean(NAVIGATION_ROUTES.find(route => route.tabId === id)))).toBe(true)
