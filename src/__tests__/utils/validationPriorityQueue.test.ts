...
describe("validationPriorityQueue", () => {
  it("computes a sorted Top 10 queue with field-level provenance", () => {
    ...
    for (const row of rows) {
      ...
      expect(row.mof).toBeTruthy()
      expect(row.pathway).toBeTruthy()
      expect(row.targetProduct).toBeTruthy()
      expect(row.evidenceCoverage).toEqual(expect.objectContaining({ score: expect.any(Number) }))
      expect(row.confidenceLevel).toMatch(/high|medium|low/)
      expect(row.keyRisks.length).toBeGreaterThan(0)
      expect(row.missingData.length).toBeGreaterThan(0)
      expect(row.suggestedNextExperiment).toBeTruthy()
      expect(row.whyNow).toBeTruthy()
      expect(row.scoreExplanation).toMatch(/Priority Score/)
