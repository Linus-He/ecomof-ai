...
describe("gas adsorption v2 records", () => {
  ...
  it("marks computed IAST selectivity with source isotherms and fit quality", () => {
    ...
    for (const record of computedRecords) {
      ...
      expect(record.isotherm.length, `${record.id}.primaryIsotherm`).toBeGreaterThanOrEqual(3)
      expect(record.secondaryIsotherm.length, `${record.id}.secondaryIsotherm`).toBeGreaterThanOrEqual(3)
      expect(record.secondaryIsothermTemperatureK, record.id).toBe(record.condition.temperatureK)
      expect(record.secondaryIsotherm.every(point => point.gas === record.secondaryGas), record.id).toBe(true)
