...
describe("versionTimelineDynamic", () => {
  it("uses version_evolution_records.json as the dynamic source for timeline, release notes, and roadmap", () => {
    ...
    expect(projectUpdates.textContent).toMatch(/Scoring audit/)
    expect(projectUpdates.textContent).toMatch(/Provenance and boundary/)
    ...
    expect(roadmap.textContent).toMatch(/Verify precursor prices/)
  })
})
