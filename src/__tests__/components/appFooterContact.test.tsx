...
describe("AppFooter contact links", () => {
  it("uses brand icons for GitHub, X, and email contact destinations", () => {
    ...
    expect(screen.queryByRole("link", { name: "在知乎关注小落生" })).not.toBeInTheDocument()
  })

  it("keeps the renamed policy destination and charter visible in the footer", () => {
    render(<AppFooter lang="zh" navigate={vi.fn()} theme={THEME_LIGHT} />)

    expect(screen.queryByRole("button", { name: "研究宪章" })).not.toBeInTheDocument()
