...
describe("home capabilities", () => {
  it("highlights platform capabilities, data foundation, validation, and research scenarios", async () => {
    ...
    expect(within(validation).getByText("白盒筛选")).toBeInTheDocument()
    expect(within(validation).getByText("证据修正")).toBeInTheDocument()
    expect(within(validation).getByText("敏感性分析")).toBeInTheDocument()
    expect(within(validation).getByText("实验标签")).toBeInTheDocument()
    expect(within(validation).getByText("Benchmark 框架")).toBeInTheDocument()
