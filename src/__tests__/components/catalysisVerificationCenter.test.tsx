...
describe("CatalysisVerificationCenter", () => {
  it("shows real gate counts, evidence graph integration, and the task queue", () => {
    ...
    expect(screen.getByTestId("catalysis-verification-kpis")).toHaveTextContent(`${database.summary.claimLocatedCount}/${database.summary.numericClaimCount}`)
    expect(screen.getByTestId("catalysis-verification-kpis")).toHaveTextContent("实验运行")
    expect(screen.getByTestId("catalysis-verification-kpis")).toHaveTextContent("条件完整运行")
    ...
    expect(screen.getByTestId("catalysis-experiment-run-ledger")).toHaveTextContent("1小时性能点")
    expect(screen.getByTestId("catalysis-experiment-run-ledger")).toHaveTextContent("0.995 mg cm-2")
    expect(screen.getByTestId("catalysis-experiment-run-ledger")).toHaveTextContent("比较字段完整")
  ...
  it("switches the evidence trace to the selected record and exposes publisher locations", () => {
    ...
    expect(screen.getByTestId("catalysis-experiment-run-ledger")).toHaveTextContent("材料表征，不适用反应性能比较")
