...
describe("OrganicAcidExperimentalActivationCenter", () => {
  it("renders readiness, hosts, Mo strategies, matrix, template, feedback, update preview, and exports", () => {
    ...
    expect(text).toMatch(/Organic Acid Experimental Activation Center/)
    ...
    expect(text).toMatch(/Specific Al-MOF hosts/)
    expect(text).toMatch(/MIL-53\\(Al\\)-like|MIL-53/)
    expect(text).toMatch(/Primary stable host candidate/)

    fireEvent.click(screen.getByRole("tab", { name: "Mo Strategies" }))
    text = bodyText()
    ...
    fireEvent.click(screen.getByRole("tab", { name: "Experiment Matrix" }))
    text = bodyText()
