...
export const ORGANIC_ACID_ALGORITHM_METHODOLOGY_VERSION = "V3.9.4"
...
function routeName(route = {}) {
  const host = safeText(route.hostMof, "Al-MOF")
  const guest = safeText(route.guestMetal, "Mo")
...
export const ORGANIC_ACID_ALGORITHM_FORMULAS = [
  ...
  },
  {
    ...
    latex: String.raw`S_{\mathrm{host}}(h)=\sum_{k=1}^{m}w_{k}^{\mathrm{host}}x_k(h),\quad h^{*}=\arg\max_{h\in H}S_{\mathrm{host}}(h),\quad h^{*}=\mathrm{Al{-}MOF}`,
  },
  {
    id: "guest-selection",
    title: "Guest / Dopant Metal Selection",
    latex: String.raw`S_{\mathrm{guest}}(g\mid h^{*})=\sum_{j=1}^{q}w_{j}^{\mathrm{guest}}y_j(g,h^{*}),\quad g^{*}=\arg\max_{g\in G}S_{\mathrm{guest}}(g\mid h^{*}),\quad g^{*}=\mathrm{Mo}`,
  },
  {
    id: "route-definition",
    title: "Host-Guest Route Definition",
    latex: String.raw`r=(h,g,t),\quad r^{*}=(\mathrm{Al{-}MOF},\mathrm{Mo},\mathrm{doping/post{-}modification/bimetallic})`,
  },
  {
    id: "hgcps",
  ...
  },
  {
    ...
    latex: String.raw`r^{*}=\arg\max_{r\in R}\mathrm{HGCPS}(r),\quad r^{*}=\mathrm{Al{-}MOF+Mo},\quad r^{*}\neq\mathrm{final\ catalytic\ proof}`,
  },
  {
    id: "route-hypothesis",
...
export function buildOrganicAcidAlgorithmMethodology(input = {}) {
  ...
  const dynamicContext = {
    ...
    selectedHost: safeText(selectedHost.displayName || topRoute.hostMof, "Al-MOF"),
    selectedGuest: safeText(selectedGuest.guestMetal || topRoute.guestMetal, "Mo"),
  ...
  const sectionSpecs = [
    ...
    },
    {
      ...
      limitation: "Al-MOF is not final best catalyst proof; pristine host activity remains a control.",
    },
    {
      id: "guest-selection",
      ...
      explanation: `${dynamicContext.selectedGuest} is selected after the host is fixed, so Mo is a guest / dopant / activity-compensation metal under Al-MOF, not a Mo-MOF optimum claim.`,
      explanationZh: `${dynamicContext.selectedGuest} 在主体确定后筛出，因此 Mo 是 Al-MOF 主体下的客体 / 掺杂 / 活性补偿金属，不是 Mo-MOF 最优声明。`,
      ...
      limitation: "Mo loading, oxidation state, and local coordination require characterization.",
    },
    {
      id: "hgcps",
      title: "Host-Guest Complementary Pathway Score",
    ...
    },
    {
      ...
      explanation: "Sensitivity checks +/-20% single-factor perturbation; ablation removes guest compensation, host stability, complementarity, evidence confidence, risk retention, pristine Al-MOF only, and Mo contribution.",
      explanationZh: "敏感性分析检查单因子 ±20% 扰动；消融项覆盖去除客体补偿、主体稳定性、主客体互补、证据置信、风险保留、仅 pristine Al-MOF、移除 Mo 贡献。",
...
export function buildOrganicAcidAlgorithmFormulaJson(methodology) {
  ...
  return {
    ...
    boundary: "Al-MOF + Mo is a high-priority experimental hypothesis, not final catalytic proof.",
  }
}
