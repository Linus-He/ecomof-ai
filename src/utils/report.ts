...
export function buildMofMarkdownReport(record, options = {}) {
  ...
  return [
    ...
    `- ${lang === "zh" ? "报告用途" : "Report use"}: ${lang === "zh" ? "候选优先级与研究讨论（candidate priority / research discussion）" : "candidate priority / research discussion"}`,
    `- ${lang === "zh" ? "范围" : "Scope"}: ${options.scopeLabel || (lang === "zh" ? "单个 MOF 记录" : "single MOF record")}`,
    "",
    lang === "zh" ? "## 2. 候选 / MOF 摘要" : "## 2. Candidate or MOF summary",
    "",
    lang === "zh" ? "| 字段 | 数值 / 内容 |" : "| Field | Value |",
    "| --- | --- |",
    ...summaryRows.map(([key, value]) => `| ${key} | ${reportList(value)} |`),
    "",
    lang === "zh" ? "## 3. 关键描述符" : "## 3. Key descriptors",
    "",
    lang === "zh" ? "| 描述符 | 数值 | 来源状态 | 来源 / 条件 | 证据状态 |" : "| Descriptor | Value | Provenance status | Source / condition | Evidence status |",
    "| --- | --- | --- | --- | --- |",
    ...reportDescriptorRows(record, lang),
    "",
    lang === "zh" ? "## 4. 字段级来源与证据状态" : "## 4. Field-level provenance and evidence status",
    "",
    lang === "zh"
    ...
    "",
    lang === "zh" ? "## 5. 数据缺口" : "## 5. Data gaps",
    "",
    `- EcoScreen: ${gapText(gaps.ecoScreen)}`,
    `- CatalysisLab: ${gapText(gaps.catalysisLab)}`,
    "",
    lang === "zh" ? "## 6. 限制与科学边界" : "## 6. Limitations and scientific boundary",
    "",
...
export function buildMofCandidateMarkdownReport(candidates = [], options = {}) {
  ...
  const summaryLines = rows.map((record, index) => {
    ...
    return [
      ...
      "",
      lang === "zh" ? "| 描述符 | 数值 | 来源状态 | 证据状态 |" : "| Descriptor | Value | Provenance status | Evidence status |",
      "| --- | --- | --- | --- |",
  ...
  return [
    ...
    lang === "zh" ? "## 1. 任务上下文 / 数据模式" : "## 1. Task context / data mode",
    "",
    `- ${lang === "zh" ? "数据路线" : "Data route"}: ${options.dataMode || "public/data"}`,
    ...
    "",
    lang === "zh" ? "## 3. 关键描述符 / 4. 来源状态 / 5. 数据缺口" : "## 3. Key descriptors / 4. Provenance / 5. Data gaps",
    "",
    ...summaryLines,
    "",
    lang === "zh" ? "## 6. 限制与科学边界" : "## 6. Limitations and scientific boundary",
