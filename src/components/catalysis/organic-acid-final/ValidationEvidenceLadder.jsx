        ...
        {rows.map(row => {
          ...
          return (
            <article key={row.level} style={{ background: open ? t.badgeInfoBg : t.surface, border: `1px solid ${open ? t.accentText : t.border}`, borderRadius: 10, display: "grid", gap: 8, minWidth: 0, padding: 11 }}>
              ...
              >
                <span style={{ alignItems: "center", background: t.panel, border: `1px solid ${t.border}`, borderRadius: 6, color: t.accentText, display: "inline-flex", fontSize: 12, fontWeight: 900, height: 32, justifyContent: "center", width: 32 }}>{row.level}</span>
              ...
              {open ? (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 7, paddingLeft: 42 }}>
                  {(lang === "zh" ? row.evidenceZh : row.evidence).map(item => (
                    <span key={item} style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 6, color: t.muted, fontSize: 11.5, fontWeight: 780, padding: "5px 8px" }}>
