        ...
        <div style={{ display: "grid", gap: 14 }}>
          {isDatabaseIndexTrace ? (
            <section style={{ background: t.badgeWarnBg, border: `1px solid ${t.warn}`, borderRadius: 10, color: t.muted, display: "grid", fontSize: 12.3, gap: 5, lineHeight: 1.5, padding: 10 }}>
              ...
              <span>{text(lang, "大规模数据追踪在索引层进行摘要，避免审计信息过载。Trace 覆盖数据清单、汇总、结构审阅样本、当前加载分片与当前打开详情。", "Large-scale trace is summarized at index level to keep the audit readable. Trace covers the data manifest, summaries, structural-review sample, current loaded part, and current opened detail.")}</span>
            </section>
          ) : null}
