...
export function DatabaseIndexSkeleton({ lang, t }) {
  return (
    <section style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 12, display: "grid", gap: 10, padding: 15 }}>
      <div style={{ color: t.accentText, fontSize: 10.5, fontWeight: 900, textTransform: "uppercase" }}>{text(lang, "CoRE 2024 CR 真实记录索引", "CoRE 2024 CR Real Record Index")}</div>
      <strong style={{ color: t.textStrong, fontSize: 18 }}>{text(lang, "正在加载结构索引…", "Loading structure index…")}</strong>
