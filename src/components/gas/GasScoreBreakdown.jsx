// @ts-nocheck
import { ChemicalText, FONT_MONO } from "../../shared"
  ...
  return (
    <div style={{ display: "grid", gap: compact ? 6 : 8 }}>
      ...
      <div style={{ alignItems: "center", display: "grid", gridTemplateColumns: compact ? "104px minmax(0, 1fr) 42px" : "148px minmax(0, 1fr) 52px", gap: 8 }}>
        ...
        <span style={{ color: t.warn, fontFamily: FONT_MONO, fontSize: compact ? 10.5 : 11.5, fontWeight: 850, textAlign: "right" }}>-{formatNumber(riskPenalty)}</span>
      </div>
      {!compact ? (
        <div style={{ color: t.subtle, fontSize: 11, lineHeight: 1.45 }}>
          <ChemicalText value={record.dataType || "demo"} /> · Evidence {record.evidenceLevel || "C"}
        </div>
      ) : null}
    </div>
  )
