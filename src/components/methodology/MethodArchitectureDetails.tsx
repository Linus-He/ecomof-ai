...
const MODULE_ARCHITECTURE = {
  ...
  "catalysis-lab": {
    ...
    introZh: "催化模块从 DOI 核验反应记录开始，把反应事实、运行条件、活性相证据、结构身份和验证任务分层保存。底层记录可供催化总览、有机酸工作台、最终筛选和数据库索引共同读取，但不会把条件不兼容的论文性能混成排名。",
    introEn: "Catalysis starts from DOI-verified reaction records and keeps reaction facts, operating conditions, active-phase evidence, structure identity, and validation tasks in separate layers. The same bottom-layer records can serve the Catalysis overview, Organic Acid workspace, Final Screening, and database index without ranking incompatible literature conditions.",
    blocks: [
      {
        titleZh: "DOI 核验反应记录",
        titleEn: "DOI-verified reaction records",
        implementationZh: "catalysis_reaction_records_v1 按统一 schema 保存论文 DOI、催化剂与前驱体身份、反应和产物、条件组合、来源报道指标、字段来源位置及核查日期。数值只有在出版方页面、DOI 页面或机构知识库中实际提取到时才写入。",
        implementationEn: "catalysis_reaction_records_v1 follows one schema for article DOI, catalyst and precursor identity, reaction and product, condition tuple, source-reported metrics, field source locations, and verification date. Numeric values are stored only when actually extracted from a publisher, DOI, or institutional-repository page.",
        ...
        outputsZh: "带字段级溯源、缺失状态和核查日期的底层反应记录。",
        outputsEn: "Bottom-layer reaction records with field provenance, missing-value status, and verification date.",
        boundaryZh: "DOI 只核验论文身份；无法从已核来源确认的数值必须留空并标记 not-extracted。",
      ...
      },
      {
        ...
        implementationZh: "反应记录通过精确 CSD Refcode、CCDC 编号、canonicalId 或来源一致的人工核验别名连接 mof_identity_registry；连接成功后可复用现有结构描述符、气体数据库、催化总览、有机酸工作台、最终筛选和验证任务系统。未解析的文献身份仍可浏览，但不会参与结构级合并。",
        implementationEn: "Reaction records join mof_identity_registry through an exact CSD Refcode, CCDC identifier, canonicalId, or provenance-matched curated alias. Linked records can reuse existing structure descriptors, gas data, Catalysis overview, Organic Acid workspace, Final Screening, and validation-task surfaces. Unresolved literature identities remain browsable but do not enter structure-level merges.",
