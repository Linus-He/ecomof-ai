  ...
  "catalysis-lab": {
    ...
    blocks: [
      ...
      },
      {
        ...
        implementationZh: "catalysis_reaction_database_v2 将来源文档、Crossref 响应快照、反应记录、实验运行、催化剂状态、条件组合、指标声明和证据项拆成独立表。同一论文中的峰值性能、分电流与稳定性试验分别保存，避免把不同电位或时长误合并。每次 DOI 刷新保存题名/期刊/年份匹配、更新关系、时间戳和快照哈希；这只核验论文身份，不替代数值声明核验。",
        implementationEn: "catalysis_reaction_database_v2 separates source documents, Crossref response snapshots, reaction records, experiment runs, catalyst states, condition sets, metric claims, and evidence items. Peak performance, partial-current, and stability runs from the same paper remain separate so different potentials or durations are not merged. DOI refreshes verify article identity, not numeric claims.",
        ...
        outputsZh: "可复现 DOI 快照、论文—实验运行—条件—声明底层表，以及每条声明从来源确认到图表精确定位的核验等级。",
        outputsEn: "Reproducible DOI snapshots, normalized article-run-condition-claim tables, and an L1-L4 level for every claim.",
      ...
      },
      {
        ...
        implementationZh: "证据分为四级：来源已确认、仅核对摘要、全文已核对、已定位到图表、章节或补充材料。系统按运行类型选择条件门：稳态性能检查电解池、电解液、控制条件、时长、载量与定量方法；极化曲线改查扫描方案，TOF 改查活性位归一化；材料表征标为不适用，不生成反应条件补录任务。训练与推荐还必须通过结构身份、许可和活性相门槛。",
        implementationEn: "L1 identifies the source, L2 covers abstract review, L3 full-text review, and L4 requires a precise location. Eligibility uses a run-type-specific condition gate: steady performance, polarization, TOF, and non-reaction characterization do not share one checklist. Training and recommendation additionally require identity, license, and active-phase clearance.",
        inputsZh: "论文元数据、声明位置、运行类型对应的比较条件、精确身份、许可和活性相状态。",
        inputsEn: "Article metadata, claim locations, run-type-specific comparison fields, exact identity, license, and active-phase status.",
        outputsZh: "使用范围矩阵、条件完整运行清单、来源与核验路径、待核原因和核验事项清单。",
      ...
      },
      {
        ...
        implementationZh: "每个实验运行独立保存电解池、电解液、电位参比、控制模式、持续时间、负载基准与定量方法；来源直接报告与按电极制备步骤计算的负载明确区分。活性相层分别记录原始 MOF、前驱体、衍生材料、原位重构相和反应后相，并连接原位光谱、反应后表征或 DFT 证据。",
        implementationEn: "Each experiment run stores cell, electrolyte, potential reference, control mode, duration, loading basis, and quantification separately; source-reported and preparation-derived loadings remain distinct. The active-phase layer separates pristine MOF, precursor, derived material, in situ reconstruction, and post-reaction phase.",
        ...
        boundaryZh: "只有同一实验运行、同一条件组合下的指标才能派生分电流或进入定量比较；前驱体的 CCDC 编号不得转移给电化学重构后的活性材料。",
        boundaryEn: "Only metrics from the same experiment run and condition tuple may derive partial current or enter quantitative comparison; a precursor CCDC identifier must not be transferred to an electrochemically reconstructed active material.",
      },
      {
        titleZh: "数据库连接与功能复用",
        titleEn: "Database joins and feature reuse",
        implementationZh: "结构身份解析只接受准确的 CSD Refcode、CCDC 编号、标准结构标识或经来源核对的别名。出版方明确属于催化材料本身的晶体编号可以解析结构身份，但只有本地身份库存在对应记录时才标为已连接；前驱体编号不得转移给衍生或重构活性材料。证据关系图连接论文、催化剂状态、反应、声明、证据和待核事项。",
