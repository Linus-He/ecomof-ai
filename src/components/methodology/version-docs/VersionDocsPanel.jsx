    ...
    <section id="methodology-literature-library" style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 12, display: "grid", gap: 12, padding: 14, scrollMarginTop: 118 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {filters.map(([id, en, zh]) => {
          ...
          return (
            <button data-selected={active} key={id} type="button" onClick={() => setFilter(id)} style={{ background: active ? t.accent : t.surface, border: `1px solid ${active ? t.accent : t.border}`, borderRadius: 6, color: active ? t.buttonText || "#fff" : t.textStrong, cursor: "pointer", fontSize: 11.5, fontWeight: 900, padding: "6px 9px" }}>
              {text(lang, zh, en)}
            </button>
          )
        })}
